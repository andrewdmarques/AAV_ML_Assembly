...
def predictnn(log_file, X, y, parameters):
    """
    ...
    print(np.around(accuracy, decimals=4), end = '')
    with open(log_file, 'a') as outf:
        outf.write(str(np.around(accuracy, decimals=4)))
    return p, accuracy
...
def L_layer_model(log_file, X, Y, X_test, Y_test, layers_dims, learning_rate = 0.1, num_iterations = 3000, print_cost=False):
	...
	print('\nIter:\tTr_cost\tTe_cost\tTr_acc\tTe_acc')
	for i in range(0, num_iterations):
		...
		previous_te_acc = float(0)
		if print_cost and i % 100 == 0 or i == num_iterations:
			print (str(i) + '\t', end = '')
			# Print trainingL_model_forward( accuracy.
			print (str(np.around(cost, decimals=4)) + '\t', end = '')
			# Print testing accuracy.
			AL_t, caches_t = L_model_forward(X_test, parameters)
			cost_test = compute_cost(AL_t, Y_test)
			print (str(np.around(cost_test, decimals=4)) + '\t', end = '')
			#Record the costs
			costs_train.append(cost)
			costs_test.append(cost_test)
			outf = open(log_file, 'a')
			outf.write(str(i) + '\t' + str(np.around(cost, decimals=4)) + '\t' + str(np.around(cost_test, decimals=4)) + '\t')
			outf.close()
			#Record the accuracies
			predictions_train, accuracy_train = predictnn(log_file, train_x, train_y, parameters)
			print('\t', end = '')
			outf = open(log_file, 'a')
			outf.write('\t')
			outf.close()
			pred_testm, accuracy_test = predictnn(log_file, test_x, test_y, parameters)
			current_te_acc = float(np.around(accuracy_test, decimals=4))
			print('')
			outf = open(log_file, 'a')
			outf.write('\n') 
			outf.close()
			if current_te_acc == previous_te_acc:
				break
			previous_te_acc == current_te_acc
		# Update parameters.
...
#Prime the log file.
log_file = str(title + '_log.txt')
with open(log_file, 'w+') as outf:
	outf.write('Settings: ' + title + '\nIter:\tTr_cost\tTe_cost\tTr_acc\tTe_acc\n')

# Train the L-layer neural network.
parameters, costs_train = L_layer_model(log_file, train_x, train_y, test_x, test_y, layers_dims, learning_rate, num_iterations, print_cost = True)
